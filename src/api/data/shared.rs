...
pub enum SharedField {
    ...
    /// Any unrecognized field descriptor for set 0xFF.
    Unknown {
        descriptor: u8,
    },
}
...
impl SharedField {
    ...
    pub fn parse(descriptor: u8, bytes: &[u8]) -> Result<Self, Error> {
        Ok(match descriptor {
            EventSource::DESCRIPTOR => Self::EventSource(EventSource::from_bytes(bytes)?),
            Ticks::DESCRIPTOR => Self::Ticks(Ticks::from_bytes(bytes)?),
            DeltaTicks::DESCRIPTOR => Self::DeltaTicks(DeltaTicks::from_bytes(bytes)?),
            GpsTimestamp::DESCRIPTOR => Self::GpsTimestamp(GpsTimestamp::from_bytes(bytes)?),
            DeltaTime::DESCRIPTOR => Self::DeltaTime(DeltaTime::from_bytes(bytes)?),
            ReferenceTimestamp::DESCRIPTOR => {
                Self::ReferenceTimestamp(ReferenceTimestamp::from_bytes(bytes)?)
            }
            ReferenceTimeDelta::DESCRIPTOR => {
                Self::ReferenceTimeDelta(ReferenceTimeDelta::from_bytes(bytes)?)
            }
            ExternalTimestamp::DESCRIPTOR => {
                Self::ExternalTimestamp(ExternalTimestamp::from_bytes(bytes)?)
            }
            ExternalTimeDelta::DESCRIPTOR => {
                Self::ExternalTimeDelta(ExternalTimeDelta::from_bytes(bytes)?)
            }
...
pub struct DeltaTime {
    /// Time delta [seconds]
    pub dt_s: f64,
    pub valid_flags: DeltaTimeValidFlags,
}
impl DeltaTime {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        ...
        Ok(Self {
            dt_s: b.read_f64(),
            valid_flags: DeltaTimeValidFlags(b.read_u16()),
        })
    }
}

/// Bitflags for (0xFF,0xD4) Delta Time valid flags.
#[derive(Debug, Copy, Clone, Eq, PartialEq)]
pub struct DeltaTimeValidFlags(pub u16);
impl DeltaTimeValidFlags {
    pub const NONE: Self = Self(0x0000);
